...
export default {
	...
	methods:{
		handleCurrentChange(val) {
			console.log(`当前页: ${val}`);
			let valnum = `${val}` * 20;
			this.$http.get("https://cosplay.it7e.com/v1/posts?access_token=" + this.token + "&reqtype=2&query=Status%3A0&sortby=Id&order=desc&limit=20&offset=" + valnum).then(function(res){
	...
	created(){
		...
		this.$http.get("https://cosplay.it7e.com/v1/posts?access_token=" + this.token + "&reqtype=2&query=Status%3A0&sortby=Id&order=desc&limit=20").then(function(res){
			for(let i = 0; i < res.data.data.length; i++){
				if(res.data.data[i].Status == 0){
                    res.data.data[i].Status = false;
                    this.userData.push(res.data.data[i])
